...
/**
 * Interface implemented by classes that load {@code EtcdConfigSource}'s internal configuration.
 *
...
interface EtcdConfig {
  ...
  /**
   * Gets the list of endpoints for an etcd cluster. This property overrides {@code host} and {@code
   * port} if provided separately.
   *
  ...
  /**
   * If {@code true}, then the configuration source will watch for changes to any previously read
   * properties.
   *
  ...
  /**
   * Gets the configuration source's key prefix (if any).
   *
   * @return The key prefix or the empty string.
   */
  String getKeyPrefix();
}
